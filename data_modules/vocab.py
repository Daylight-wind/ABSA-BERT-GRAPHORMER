...
class Vocab(object):
    def __init__(self, config, min_freq=1, special_token=['<PADDING>', '<OOV>'], max_size=None):
        ...
        and update based on minimum frequency and maximum size  #最小频率和最大尺寸
        :param config: helper.configure, Configure Object
        :param min_freq: int, the minimum frequency of tokens 把出现次数少于2次的低频率词元视为相同的未知词元即视为'<unk>'
        :param special_token: List[Str], e.g. padding and out-of-vocabulary
        :param max_size: int, maximum size of the overall vocabulary
        """
        logger.info('Building Vocabulary....')   # 打印日志信息
        ...
        if not os.path.isdir(self.config.vocabulary.dir):
            os.system('mkdir ' + str(self.config.vocabulary.dir))   # mkdir命令的主要功能为创建一个或多个空目录
        token_dir = os.path.join(self.config.vocabulary.dir, self.config.vocabulary.vocab_dict)   # 生成词表vocab中的word.dict
        label_dir = os.path.join(self.config.vocabulary.dir, self.config.vocabulary.label_dict)  # 生成词表vocab中的label.dict
        vocab_dir = {'token': token_dir, 'label': label_dir}
        if os.path.isfile(label_dir) and os.path.isfile(token_dir):  # 判断label,token是不是目录
