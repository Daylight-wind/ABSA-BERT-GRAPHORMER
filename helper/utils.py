...
def get_hierarchy_relations(hierar_taxonomy, label_map, root=None, fortree=False):
    ...
    """
    label_tree = dict()  # 在下一步把root拷贝给了label_tree,所以label_tree变成了一个 TREE类，
    label_tree[0] = root  # 把root 属于的树结构给了label_tree
    hierar_relations = {}
    with codecs.open(hierar_taxonomy, "r", "utf8") as f:  # 读取层级文件，想要读取层级信息codecs.open是将源文件解码为Unicode之后再进行操作，用于解决编码不统一的问题
        for line in f:
            ...
            if fortree:
                ...
                for child in children_label_ids:
                    ...
                    child_tree = Tree(child)    # 初始化一个Tree类
                    parent_tree.add_child(child_tree)
                    label_tree[child + 1] = child_tree     # todo:为什么这里都要+1？
    if fortree:
        return hierar_relations, label_tree
    else:
        return hierar_relations   #层级关系树
