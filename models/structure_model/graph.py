...
class BartAttention(nn.Module):
    ...
    def forward(
        ...
        # get query proj
        query_states = self.q_proj(hidden_states) * self.scaling # hidden_states（8，130，300）
        # get key, value proj
...
class GraphEncoder(nn.Module):
    def __init__(self, config, graph=False, layer=1, data_path=None, threshold=0.01, tau=1):
        ...
        self.config = config
        self.tau = tau
        # ToDo： 修改路径,这里需要处理数据的部分
        self.label_dict = torch.load('/Users/a123456/Code/fei01bert_graphmer/data_modules/bert_value_dict.pt')
        ...
        self.label_name = self.tokenizer(self.label_name, padding='longest')['input_ids']  # 在这一步，label从汉字变成了ids  ;就是转换成101....的格式发现padding=True等价于padding="longest"，只对于句子对任务起作用
        ...
        if graph:
            # todo:修改文件路径
            label_hier = torch.load("/Users/a123456/Code/fei01bert_graphmer/data_modules/slot.pt")
    ...
    def forward(self, inputs_embeds, attention_mask):
        ...
        # label_emb = (label_emb * label_mask.unsqueeze(-1)).sum(dim=1) / label_mask.sum(dim=1).unsqueeze(-1) #(141,10,768)-->(141,768) label_emb * label_mask.unsqueeze(-1)-->(141,10,768)把填充的emb置为0，对于3维的，dim=1是按照x轴进行累加，对于二维的，dim=1是y轴，所以 label_mask.sum(dim=1)可以获取每个标签的有效长度,此处把每个标签对应的有效的词的embedding做了累加求平均
        ...
        #
        # extra_attn = None
        # # cross_attn_mask是（3，512，141），矩阵中如果是pad，则为0，如果是真实的输入，是1
        # self_attn_mask = (label_attn_mask * 1.).t().mm(label_attn_mask * 1.).unsqueeze(0).unsqueeze(0) # (label_attn_mask * 1.).t()-->(141,1)*(1,141)=(141,141)-->(1,1,141,141)   .t()是转置的意思
        # # cross_attn_mask = (attention_mask * 1.).unsqueeze(-1).bmm(
        # #     (label_attn_mask.unsqueeze(0) * 1.).repeat(attention_mask.size(0), 1, 1)) # repeat 表示想在第几维上扩张几倍  (attention_mask * 1.) 则true为1，false为0，(attention_mask * 1.).unsqueeze(-1)：(3,512)-->(3,512,1)  ,(label_attn_mask.unsqueeze(0) * 1.).repeat(attention_mask.size(0), 1, 1) -->(3,1,141),其中每个batch是一样的
        ...
        label_emb=inputs_embeds # label_emb(32*72*768)
        expand_size = label_emb.size(-2) // self.label_name.size(0)

        extra_attn = self.distance_embedding(self.distance_mat) + self.edge_embedding(self.edge_mat).sum(
            dim=1) / (self.distance_mat.view(-1, 1) + 1e-8) # distance_embedding是随机初始化的（20，1）  extra_attn(1*5184*1)
        extra_attn = extra_attn.view(self.label_num, 1, self.label_num, 1).expand(-1, expand_size, -1,expand_size)  # extra_attn(72*1*72*1)
        extra_attn = extra_attn.reshape(self.label_num * expand_size, -1)#（130，130）   #extra_attn(72，72)
        # elif GRAPH == 'GCN' or GRAPH == 'GAT':
        #     extra_attn = self.edge_list
        for hir_layer in self.hir_layers:
            label_emb = hir_layer(label_emb, extra_attn) #(8,130,768)
