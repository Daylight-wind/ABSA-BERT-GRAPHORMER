...
class StructureEncoder(nn.Module):
    def __init__(self,
                 ...
                 graph_model_type):
        """
        Structure Encoder module:初始化函数
        ...
        self.hierarchical_label_dict, self.label_trees = get_hierarchy_relations(os.path.join(config.data.data_dir,
                                                                                 ...
                                                                                 root=self.root,
                                                                                 fortree=True)  # 获取层级信息
        hierarchy_prob_file = os.path.join(config.data.data_dir, config.data.prob_json)  # 获取文件名
        f = open(hierarchy_prob_file, 'r')
        hierarchy_prob_str = f.readlines()
        f.close()
        self.hierarchy_prob = json.loads(hierarchy_prob_str[0])
        self.node_prob_from_parent = np.zeros((len(self.label_map), len(self.label_map)))  # 初始化定义为0矩阵
        self.node_prob_from_child = np.zeros((len(self.label_map), len(self.label_map)))  # 初始化定义为0矩阵

        for p in self.hierarchy_prob.keys():   # 取出父节点
            if p == 'Root':
                continue
            for c in self.hierarchy_prob[p].keys():  # 取出子节点
                # self.hierarchy_id_prob[self.label_map[p]][self.label_map[c]] = self.hierarchy_prob[p][c]
                self.node_prob_from_child[int(self.label_map[p])][int(self.label_map[c])] = 1.0   # 0,1矩阵
                self.node_prob_from_parent[int(self.label_map[c])][int(self.label_map[p])] = self.hierarchy_prob[p][c]   # 将子节点的矩阵传进去
